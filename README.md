...
* **Highlight Group Optimal** identifies the words in the currently displayed word list, or in the current vocabulary, that when used as a guess against the current word list will result in remaining choice lists containing the smallest number of choices ***when that guess is wrong***. In other words, any word you pick from a given list has an even chance of being the solution, but some picks leave you fewer remaining words to choose from, like only one word. It is possible for one word in a word list to leave only one word remaining for every which way that word was wrong. You would solve the Wordle in your next guess under that condition. **Highlight Group Optimal** identifies that word.
...
* The max p value is the maximum probability discovered amongst the optimal words. Refer to the verbose output to see which optimal word has the maximum probability. P values vary when guess words share the same average clue pattern group size but vary in their maximum clue pattern group size.
