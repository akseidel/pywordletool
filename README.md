...
* **Show Group Optimal** highlights the word or words that result in the lowest average resulting group size when that word is used as a guess. This analysis is performed on the current displayed words. The computation process is intensive and may take time to finish. The group optimal guess word is likely to result in remaining words of smaller choice quantity, thus increasing chances to find the solution in fewer guesses.    
