...
* Guess words can leave fewer remaining words to choose from than what other guess words leave. It is possible for a guess word to leave only one word remaining for every which way that word was wrong. You would solve the Wordle in your next guess under that condition. Optimal guess words might be in the **Words Showing** (an "in-pool" guess) but are more likely selected from the other vocabulary sources (an "out-of-pool" guess). **Optimals** identifies those words.
