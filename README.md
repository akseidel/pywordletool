...
Wordle is commonly described to be a word and letter guessing puzzle. That description leads one away from understanding the true nature of how every guess operates to solve the puzzle. The guess operation acts to reduce the number of possible solutions. 

Better guesses are those identified by a strategy that examines the most likely resulting number of possible solutions. This is not guessing a word or trying to identify letters. It is examining all the possible outcomes resulting from every guess in a way that measures how well the possible solutions are spread out. That measure identifies the better, **optimal** guesses.

"Better guess" is not necessarily "best guess". There are secondary, less influential factors other than numbers of solutions in guess outcomes that come into play that might isolate the actual "best guess" in some circumstances. Much more often than not "better guess" is "best guess". It is also often the case that there is no end result difference.

* **Optimals** identifies words likely to result in the least number of remaining solutions when used as a guess against the current **Words Showing** solutions word list. The guess words can be set to be from the words in the current words **Showing**, or in one of the vocabulary sources: **Classic**, **Classic+** or **Large**.  
* Any word from the **Words Showing** list used as a guess has a chance of being the solution. That chance is related to the number of **Words Showing**.
* Guess words can leave fewer remaining words to choose from than what other guess words leave. It is possible for a guess word to leave only one word remaining. You would solve the Wordle in your next guess under that condition. An optimal guess word or words might be in the **Words Showing** (an "in-pool" guess). Such optimal guesses are more likely selected from the other vocabulary sources (an "out-of-pool" guess). **Optimals** identifies those words.
* Outcome word sets are words that share a unique clue pattern. Thus, an outcome is an abstract that is both the clue pattern and the outcome word collection. A separate window opens to report all the outcome pattern information when the **Verbose** checkbox is checked.
* Outcomes identified with **Optimals** or **Outcome Driller**, in verbose mode, can be used to calculate the expected number of steps to solve (**ESS**) associated with any guess word. See [EXPECTEDSTEPS_Readme.md](EXPECTEDSTEPS_Readme.md) for more information. Calculating the **ESS** can augment understanding Wordle dynamics, but it is not necessary for selecting good guess words.
...
* The actual best Wordle play is the word having the least **ESS**. **Wordle Helper** does not calculate **ESS**. It does produce the data necessary for that calculation but experience will show that **Entropy** is a very, very close substitute. **Entropy** reveals the words that result in the most evenly distributed outcome sizes. **Entropy** is based on the word counts in outcomes. The **ESS** calculation looks at outcome word counts **AND** word interactions between words within outcomes. Word interactions within an outcome can have the effect of decreasing the actual outcome size. For example, a 14 word outcome can take only two plays to solve. The **Entropy** calculation would not know that. The **ESS** calculation would know that.
* The effect word interactions between words within outcomes has on the **ESS** is often not large enough to contradict the **Entropy** prediction. When it does, **Entropy** is the runner up. Since both guess word selections will be very good and since it is the solution that actually has the authority, the lengthy **ESS** calculation is not needed. The Wordle is more likely to solve in the same number of steps when using either method. Only the "skill" will be different.
* The **min-max** value is the minimum size of the maximum outcome sizes in guess words identified to yield the largest number of word outcomes. Many guess words yield the same largest number of word outcomes as do other guess words, but the word outcome sizes can vary. One wants to know the smallest maximum outcome size in the outcomes analysis summary. The **min-max** value of 1 means the guess solves the solution in 2 steps for an out-of-pool guess regardless of the remaining words count or less than 2 steps for an in-pool guess. The **min-max** value of 2 where the outcome number is 1 less than the remaining word count ("N-1") always solves in 2 steps for in-pool guesses regardless of the remaining words count.
...
* Larger **Entropy** values typically, but not always, go with the largest outcome qty having the smallest **min-max** value and smallest **p2** value. The NYT WordleBot selects by its **ESS** calculation, which is closely coupled to the **Entropy** value.
