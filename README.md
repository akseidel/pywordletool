...
* The **Condensed** listing shows Wordle **Standard (Default) Mode** guesses as ranked by **Entropy**. There is not a way to filter out words so that only **Hard Mode** words are shown. However, it is often possible to use the **Find** feature to highlight the words that comply with **green** Wordle clues such that it is easy to then spot those words also having the correct **yellow** letters. The **Find** feature operates on regex patterns. For example, if the letter **i** is required as the 2nd letter and the letter **r** is required as the last letter, then "**^.i..r**" is the regex pattern that will cause all words like **timer**, **sitar** and **tiger** to be highlighted. Note that the "**^**" is required. Its regex meaning is "starting with". The "**.**", or dots, mean "any character." 
